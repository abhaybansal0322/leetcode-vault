class Solution {
    const int MOD = 1e9+7;
    int t[201][201][201];
    int helper(vector<int>& nums, int i, int first, int second) {

        if(i == nums.size()) {

            bool bothNonEmpty = (first!=0) && (second!=0);
            bool sameGcd = (first == second);

            return t[i][first][second] = (bothNonEmpty && sameGcd) ? 1 :0;
        }

        if(t[i][first][second] != -1) return t[i][first][second];

        int skip = helper(nums,i+1,first,second);
        int take1 = helper(nums,i+1,__gcd(first,nums[i]),second);
        int take2 = helper(nums,i+1,first,__gcd(second,nums[i]));

        return t[i][first][second] = (0LL+skip+take1+take2)%MOD;
    }
public:
    int subsequencePairCount(vector<int>& nums) {
        memset(t,-1,sizeof(t));
        return helper(nums,0,0,0);
    }
};